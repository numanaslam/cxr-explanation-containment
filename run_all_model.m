...
if TEST_MODE
    ...
    maxImages = 132; nRuns = 3;   % paper: whole set, 5 LIME runs / image
...
for mi = 1:numel(models)
    ...
    try
        writetable(cell2table(master, 'VariableNames', ...
            {'Model','Condition','Config','Precision','LiftOverChance','pValue','IoU'}), ...
            fullfile(resultsDir,'containment_combined.csv'));
        writetable(cell2table(accRows, 'VariableNames', ...
            {'Model','Condition','N','Accuracy','Sensitivity','Specificity'}), ...
            fullfile(resultsDir,'classification_accuracy.csv'));
