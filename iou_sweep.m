...
%% ================== models ==================
models = {'alexnet_v2.mat','vgg16_v2.mat','vgg19_v2.mat'};
...
conditions = struct('name',{},'imgDir',{},'recursive',{},'classFrom',{},'scaleMatch',{},'marginFrac',{});
conditions(end+1) = struct('name','ROI', ...
    'imgDir','C:\paper2_repo\input\annotated_gray\annotated_gray', ...
    'recursive',true,  'classFrom','folder',  'scaleMatch',false,'marginFrac',0);    % in-distribution
conditions(end+1) = struct('name','FullCXR', ...
    'imgDir','C:\paper2_repo\input\cxr_resized', ...
    'recursive',false, 'classFrom','filename','scaleMatch',false,'marginFrac',0);    % OOD, native full-frame scale
conditions(end+1) = struct('name','FullCXR-scaled', ...
    'imgDir','C:\paper2_repo\input\cxr_resized', ...
    'recursive',false, 'classFrom','filename','scaleMatch',true, 'marginFrac',0.25); % OOD, lung ~training scale (+25% context)
...
function out = processImage(net, imgPath, maskPath, isAbn, cfgs, inputSize, ...
                            ...
                            execEnv, scaleMatch, marginFrac, abnormalClass)
    nCfg = numel(cfgs);
    out = struct('ok',false,'lungFrac',0,'isAbnormal',logical(isAbn), ...
        'prec',zeros(nCfg,1),'iou',zeros(nCfg,1),'rec',zeros(nCfg,1), ...
        'dice',zeros(nCfg,1),'lift',zeros(nCfg,1),'predAbn',false,'trueAbn',logical(isAbn));
    ...
    for c = 1:nCfg
        cfg = cfgs(c);
        if strcmp(cfg.type, 'lime')
            p = zeros(nRuns,1); io = p; rc = p; dc = p;
            for r = 1:nRuns
                [~, fMap, fImp] = imageLIME(net, imgR, YPred, ...
                    'Segmentation', cfg.seg, 'NumFeatures', cfg.m, 'NumSamples', numSamples, ...
                    'ExecutionEnvironment', execEnv);
                [~, idx] = maxk(fImp, min(cfg.K, numel(fImp)));
                A = ismember(fMap, idx);
                [p(r), io(r), rc(r), dc(r)] = maskMetrics(A, gtMask);
            end
            out.prec(c)=mean(p); out.iou(c)=mean(io); out.rec(c)=mean(rc); out.dice(c)=mean(dc);
        else % gradcam
            A = gradcamMask(net, imgR, YPred, inputSize, cfg.thr, execEnv);
            [out.prec(c), out.iou(c), out.rec(c), out.dice(c)] = maskMetrics(A, gtMask);
        end
        out.lift(c) = out.prec(c) - lf;
    end
...
function A = gradcamMask(net, imgR, YPred, inputSize, thr, execEnv)
    try
        ...
        A = false(inputSize); return;   % layer auto-select failed
    end
